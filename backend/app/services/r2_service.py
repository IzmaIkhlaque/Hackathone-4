"""
AWS S3 Service (compatible with original R2 interface)
"""
import os
import json
import boto3
...
class R2Service:
    ...
    def get_chapter(self, chapter_id: str) -> str:
        """Fetches markdown content for a chapter from S3."""
        try:
            # Assumes files are stored as chapters/chapter-01.md
            key = f"chapters/{chapter_id}.md"
            response = self.s3.get_object(Bucket=self.bucket, Key=key)
            return response['Body'].read().decode('utf-8')
        except ClientError as e:
            if e.response['Error']['Code'] == "NoSuchKey":
                raise HTTPException(status_code=404, detail=f"Chapter {chapter_id} not found in S3")
            raise HTTPException(status_code=500, detail=str(e))

    def get_quiz(self, quiz_id: str) -> dict:
        """Fetches and parses quiz JSON from S3."""
        try:
            # Assumes files are stored as quizzes/quiz-01.json
            key = f"quizzes/{quiz_id}.json"
            response = self.s3.get_object(Bucket=self.bucket, Key=key)
            return json.loads(response['Body'].read().decode('utf-8'))
        except ClientError as e:
            if e.response['Error']['Code'] == "NoSuchKey":
                raise HTTPException(status_code=404, detail=f"Quiz {quiz_id} not found in S3")
            raise HTTPException(status_code=500, detail=str(e))

    def list_chapters(self):
        """Lists all chapter files in the chapters/ prefix."""
        try:
            response = self.s3.list_objects_v2(Bucket=self.bucket, Prefix="chapters/")
            return [obj['Key'] for obj in response.get('Contents', []) if obj['Key'].endswith('.md')]
...
def get_chapter(chapter_id: str) -> str:
    return _get_service().get_chapter(chapter_id)

def get_quiz(quiz_id: str) -> dict:
    return _get_service().get_quiz(quiz_id)

def list_chapters():
    return _get_service().list_chapters()
